...
- SSH 없이 폰에서도 서버 관리.
- `/관리자` → **고급 도구** → **고급 RCON**이 원격 치트 콘솔(op 레벨 4로 실행).
...
| `/관리자` | 서버 제어, 플레이어, 백업, 월드, 업데이트, 로그, 저장공간, 성능, 사고 대응, 친구 다중 계정 등록, 관리자 전용 도움말, 고급 도구를 담은 비공개 관리자 패널. |
| `/내도구` | 등록된 계정 선택, 본인 구조·위치·점수, 좌표북, 일지를 담은 비공개 친구 패널. |
...
마크닉, 좌표 이름, 일지, 공지, 원시 RCON, 허용목록처럼 텍스트가 본질인 경우에만
표시됩니다.
...
첫 화면에는 매일 쓰는 버튼(새로고침, 접속자 관리, 서버 제어, 백업, 긴급 복구,
...
`/관리자` 패널의 **플레이어** 버튼을 누르면 현재 접속자 이름이 드롭다운으로
