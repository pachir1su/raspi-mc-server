...
자주 쓰는 동작은 패널을 파고들 필요 없이 **최상위 슬래시 명령 한 번**으로
실행합니다(이슈 #79). 모든 명령은 패널 버튼과 완전히 같은 검증된 RCON 빌더를
사용하며, 응답은 본인에게만 보입니다.

| 명령 | 권한 | 기능 |
|---|---|---|
| `/명령어 <명령>` | 관리자 | 서버 콘솔 명령 하나를 그대로 실행(치트 포함). |
| `/접속자목록` | 관리자 | 현재 접속자를 즉시 표시. |
| `/전체공지 <내용>` | 관리자 | 게임 안 모두에게 공지. |
| `/시간 <낮·정오·밤·자정>` | 관리자 | 월드 시간 프리셋 변경. |
| `/날씨 <맑음·비·천둥>` | 관리자 | 월드 날씨 프리셋 변경. |
| `/무적 <접속자> [초]` | 관리자 | 대상을 일시 무적으로(기본 5초). 재생·저항·화염 저항·포화를 파티클 숨김으로 부여(이슈 #75). |
| `/무적해제 <접속자>` | 관리자 | 위에서 건 무적 효과만 제거. |
| `/지급 <접속자> <아이템> [수량]` | 관리자 | 한글 별칭 아이템 지급. |
| `/통계 <접속자>` | 관리자 | 대상의 사망·처치 수 조회. |
| `/내통계` | 친구 | 등록된 본인 계정의 사망·처치 수 조회(이슈 #68). |
| `/위키 <문서>` | 공개 | 양조·인챈트·농사 등 GitHub 도움말 문서로 이동(이슈 #71). |

접속자를 인자로 받는 명령은 **현재 접속자 자동완성**을 제공하므로 이름을 외워
칠 필요가 없습니다. 무적·통계는 파티클을 숨기고, 통계는 봇이 처음 실행된
시점부터 스코어보드로 집계됩니다(과거 기록 소급 없음). 위키 링크의 기본 주소는
저장소 문서이며 `.env`의 `MC_WIKI_BASE_URL`로 바꿀 수 있습니다.
...
하위 패널(접속자 관리·서버 제어·백업·빠른 명령·더보기·상태 진단 등)은 **새
메시지를 띄우지 않고 같은 메시지 안에서 화면을 교체**합니다(#58). 그래서 버튼을
눌러도 채팅이 쌓이지 않으며, 각 하위 화면의 **🏠 홈** 버튼으로 대시보드로
한 번에 돌아옵니다. 성능·렉 원인처럼 느린 조회는 먼저 "불러오는 중" 화면을 보인
뒤 결과로 바뀝니다.
