...
```text
/link request minecraft_name:<정확한 자바 닉네임>
```
...
`/link revoke`로 기존 연결을 해제하세요. 하나의 마크닉은 한 Discord 계정만 연결할
수 있고, 대소문자를 구분하지 않으며, 자바 에디션 형식의 1~16자 닉네임만 받습니다.

연동 승인은 마인크래프트 화이트리스트 추가가 아닙니다. 서버 입장 허용은 계속
관리자 `/whitelist add` 명령으로 따로 처리합니다.
...
| 구조/위치 조회가 플레이어를 못 찾음 | 연결된 정확한 자바 닉네임으로 현재 접속 중인지 확인. |
