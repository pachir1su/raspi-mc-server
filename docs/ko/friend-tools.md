...
# /rescue spawn이 사용하는 고정 목적지. 기본 좌표는 사용하지 않음.
...
3. **F3**을 눌러 XYZ를 확인하거나, 관리자 `/players` 위치 보기로 좌표를 확인합니다.
...
```text
/link request minecraft_name:<정확한 닉네임> edition:<Java 또는 Bedrock>
```
...
```text
/link approve user:<Discord 멤버>
```

친구는 `/link status`로 결과를 확인합니다. Discord 계정이나 마크닉이 바뀌면
`/link revoke`로 기존 연결을 해제하세요. Java 이름은 일반적인 1~16자 형식을
따르고, Bedrock 게이머태그는 공백도 받을 수 있습니다. 두 경우 모두 명령 삽입에
사용될 수 있는 문자는 거부합니다.

승인 전에 Java는 `whitelist add`, Bedrock은 Floodgate의 `fwhitelist add`를
자동 실행합니다. 서버 변경이므로 `/link approve`는 계속 `ADMIN_USER_IDS` 뒤에
있습니다. 친구 입장 허용을 위해 두 번째 작업을 할 필요는 없습니다.

## 3. 연결된 본인 계정만 구조

- `/rescue spawn`은 명령을 실행한 Discord 사용자에게 승인된 마크닉만 이동합니다.
  플레이어가 온라인이어야 합니다. 목적지는 고정 `MC_SPAWN_*` 설정이며 친구가
  대상·좌표·RCON 명령을 입력할 수 없습니다.
- `/rescue whereami`는 연결된 본인 온라인 플레이어의 차원과 XYZ만 읽습니다.

관리자의 계정 연동 승인이 이 고정 TP 한 가지에 대한 위임 권한입니다. 일반 서버
변경, 원시 `/mc`, 시작·정지, 백업, 화이트리스트, 사고 대응, 월드 관리 명령은 모두
...
```text
/place add name:<이름> dimension:<선택> x:<x> y:<y> z:<z> description:<선택> photo:<선택>
/place list
/place show name:<이름>
/place delete name:<이름>
```
...
```text
/diary add message:<내용> photo:<선택>
/diary recent limit:<1-20>
/diary show entry_id:<recent에 표시된 ID>
```
...
`/server-score`는 Paper TPS, RCON 연결, CPU 온도, 5분 부하, 메모리, HDD 여유 공간,
...
| 연동이 계속 승인 대기 | 서버장이 `/link list` 확인 후 `/link approve` 실행. |
...
로컬 RCON 또는 관리자 전용 `/mc`로만 실행하고 친구에게 이 명령 권한을 주지 않습니다.
중간 단계가 실패해도 즉시 화이트리스트를 다시 켜고 `whitelist list`로 확인한 다음
`/link approve`를 다시 실행합니다. 화이트리스트를 끈 상태로 계속 운영하지 마세요.
