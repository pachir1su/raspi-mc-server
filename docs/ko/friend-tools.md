...
친구는 `/내도구`에서 **연동 요청**을 누르고 Java/Bedrock을 고른 뒤 모달에 정확한
마크닉만 입력합니다. 서버장은 `/관리자`의 **연동 관리**에서 요청을 선택하고
**승인**을 누릅니다. 친구는 `/내도구`의 **연동 상태**로 결과를 확인합니다.
Discord 계정이나 마크닉이 바뀌면 같은 관리자 패널에서 **해제**를 누르세요.
Java 이름은 일반적인 1~16자 형식을
따르고, Bedrock 게이머태그는 공백도 받을 수 있습니다. 두 경우 모두 명령 삽입에
사용될 수 있는 문자는 거부합니다.

승인 전에 Java는 `whitelist add`, Bedrock은 Floodgate의 `fwhitelist add`를
자동 실행합니다. 서버 변경이므로 승인은 계속 `ADMIN_USER_IDS` 뒤에
있습니다. 친구 입장 허용을 위해 두 번째 작업을 할 필요는 없습니다.

## 3. 연결된 본인 계정만 구조

- `/내도구`의 **스폰 구조**는 실행한 Discord 사용자에게 승인된 마크닉만 이동합니다.
...
| 연동이 계속 승인 대기 | 서버장이 `/관리자` → **연동 관리**에서 요청을 선택하고 **승인**. |
...
관리자 연동 패널에서 **승인**을 다시 누릅니다. 화이트리스트를 끈 상태로 계속 운영하지 마세요.
