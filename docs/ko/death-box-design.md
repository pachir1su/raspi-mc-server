...
expire-hours: 0                  # 0이면 만료하지 않음(기본: 영구 보관)
max-physical-boxes-per-player: 0 # 1인당 물리 상자 상한(0이면 무제한, 기본 무제한)
...
친구 전용 서버라 두 보호 장치 모두 **기본 꺼짐**입니다. 상자 도배가 실제로
문제가 되면 그때 켜면 됩니다.

- `max-physical-boxes-per-player`: 1인당 활성 **물리(블록) 상자** 상한
  (기본 0 = 무제한). 0보다 크게 설정하면 상한 도달 뒤의 사망은 블록을 놓지
  않고 **가상 상자**로 보관되어 관리자가 `/deathbox recover`로 돌려줍니다.
- `expire-hours`: 기본 0 — 상자는 주인이 회수할 때까지 **영구 보관**됩니다.
  0보다 크게 설정하면 그 시간이 지난 상자를 매시간 정리합니다.
