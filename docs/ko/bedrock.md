...
## Discord 계정 등록과 입장 허용

서버장은 `/관리자`의 **친구 계정**에서 Discord 사용자를 고르고
**Bedrock(모바일) 추가**를 눌러 정확한 Xbox 게이머태그를 입력합니다. 같은 화면에서
Java 계정도 추가할 수 있고 Discord 사용자 한 명에게 여러 계정을 등록할 수 있습니다.
등록하면 올바른 Paper/Floodgate
화이트리스트 명령도 함께 실행되므로 별도 화이트리스트 작업이 필요 없습니다.
Floodgate는 같은 이름의 자바 계정과 충돌하지 않도록 서버 내부 이름 앞에 `.`을
붙이지만, 관리자는 평소 게이머태그만 입력합니다.
