...
점검·업데이트·장애 대응 순서입니다. 평소에는 Discord `/관리패널`을 사용하고,
...
`/플레이어`에서 접속자를 선택한 뒤 인벤토리, 좌표·차원, 체력·허기·경험치,
...
증상: `/저장공간` 실패, systemd 서비스 시작 실패, `/건강` 마운트 오류.
...
복구 전 `/백업 검증`으로 검사하고, 중요한 백업을 다른 장치에도 복사한 뒤,
접속자에게 정지를 알리고 `/백업 복구`를 실행합니다. 복구는 현재 월드를 다시
비상 백업하지만 같은 HDD의 백업은 HDD 자체 고장을 막지 못합니다.

## 안전한 업데이트

평소에는 `/업데이트 확인` → **업데이트 설치**를 사용하고, 재시작 뒤 `/업데이트 상태`를
확인합니다. 이미 받은 공식 배포 ZIP은 `/업데이트 업로드`로 보낼 수 있습니다. 소스
...
예상하지 못한 로컬 변경은 덮어쓰지 말고 먼저 백업합니다. 업데이트 후 `/관리패널`,
`/성능지표`, `/로그`를 확인하세요. 앱 업데이트는 월드를 변경하지 않으므로 매번 월드
