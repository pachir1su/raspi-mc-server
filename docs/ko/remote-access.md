...
프록시로는 게임 포트를 **실어 나르지 못합니다**. Geyser 베드락 통신은 UDP이므로
일반 Cloudflare Tunnel로도 운반되지 않습니다. 플레이어용 옵션:

- **Cloudflare Tunnel TCP** — 각 플레이어가 `cloudflared access tcp`를 실행해야
  함(친구에겐 번거로움).
- **포트포워딩** — 공유기에서 `25565/TCP`, 베드락도 쓴다면 `19132/UDP`를 파이로
  전달(친구에겐 가장 간단하며 게임 포트만 노출).
- **VPN**(Tailscale/WireGuard) — 친구를 내 tailnet에 추가하고 파이의 VPN IP로
  접속. 공개 노출이 전혀 없음. 3~4명 친구 서버엔 대개 가장 좋은 선택입니다.

> 추천: 친구 설정을 거의 없애려면 `25565/TCP`와 `19132/UDP`를 포워딩하세요.
> 친구 기기마다 VPN 설치가 괜찮다면 **Tailscale**, 관리는 **Cloudflare Access +
> 디스코드 봇**을 사용합니다.

## 보안 요약

- 플레이어에게는 게임 포트(`25565/TCP`, 필요 시 `19132/UDP`)만 노출하거나 VPN으로
  비공개 유지.
