...
```text
/link request minecraft_name:<정확한 닉네임 또는 Xbox 게이머태그> edition:<Java 또는 Bedrock>
```
...
```text
/link list
/link approve user:<친구 Discord 계정>
```

승인은 적절한 Java/Floodgate 화이트리스트에도 자동 추가합니다. 승인된 친구는 본인
연동 계정에 한해 `/rescue spawn`, `/rescue whereami`를 쓸 수 있습니다.
...
평소에는 Discord `/panel`을 쓰고, 봇이 안 될 때 Windows에서 SSH로 접속합니다.
...
- [ ] Discord `/link approve`와 본인 구조 확인
