...
Discord에서 친구는 `/내도구`의 **연동 요청**에서 에디션을 고르고 정확한 닉네임
또는 Xbox 게이머태그만 입력합니다. 서버장은 `/관리자`의 **연동 관리**에서 요청을
선택하고 **승인**을 누릅니다.

승인은 적절한 Java/Floodgate 화이트리스트에도 자동 추가합니다. 승인된 친구는 본인
연동 계정에 한해 `/내도구`의 **스폰 구조**, **내 위치**를 쓸 수 있습니다.
...
- [ ] Discord `/관리자` 연동 승인과 `/내도구` 본인 구조 확인
