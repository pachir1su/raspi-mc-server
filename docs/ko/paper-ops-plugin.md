...
Discord 스폰 귀환 버튼은 항상 제한된 `raspiops rescue` 명령을 호출합니다.
플러그인은 검증된 온라인 플레이어 정확히 한 명만 찾아 Paper의 실제 주 월드
스폰으로 이동합니다. 스폰 위치는 `/관리자` → **빠른 명령** → **스폰 지정**으로
바꿀 수 있고, 죽었을 때 리스폰 위치와 항상 같습니다. (과거의 `MC_SPAWN_X/Y/Z`
좌표 오버라이드는 두 위치가 어긋나는 원인이라 제거되었습니다.)

## 날씨 조회

`raspiops weather`는 주 월드의 현재 날씨를 `weather: clear`(맑음),
`weather: rain`(비), `weather: thunder`(뇌우)로 출력합니다. 읽기 전용이며,
Discord 관리자 패널의 번개 버튼이 비 올 때만 발동하도록 이 값을 확인합니다.
바닐라 Java에는 날씨 조회 명령이 없어 이 헬퍼를 둡니다.

## 안전 소환

`raspiops summon <플레이어> <프리셋>` 과
`raspiops villager <플레이어> <직업> <상품> <가격>` 은 `/관리자` →
**연출·소환** 하위 패널의 서버장 전용 소환을 담당합니다. 순수 명령어로는 안 되는
일 — 대상 주변에서 통과 가능한 두 칸 아래에 단단한 바닥이 있는 자리를 찾아
소환 — 을 플러그인이 처리하므로 몹이 벽에 껴서 질식하지 않습니다. 프리셋
(`creeper`, `charged_creeper`, `boom_creeper`, `buffed_zombie`,
`skeleton_squad`, `trident_drowned`, `horde`)과 조건(충전은 뇌우, 드라운드는
물)은 원시 NBT가 아니라 Bukkit API로 구현해 서버의 아이템 컴포넌트 포맷에
무관하게 유효합니다. 각 호출은 봇이 전달하는 `summon: ...` 상태 줄을 출력하며,
아무것도 안 됐을 때(안전한 자리 없음·날씨 불충족·주변에 크리퍼 없음)는 거짓
성공 대신 그 사유를 알립니다.
