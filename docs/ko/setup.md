...
```bash
.venv/bin/python -m bot.rcon 'op 내마크닉네임'
```
...
서버장은 `/관리자` → **친구 계정**에서 Discord 사용자를 고른 뒤
**Java(PC) 추가** 또는 **Bedrock(모바일) 추가**를 누르고 정확한 마크닉을 입력합니다.
같은 Discord 사용자에게 PC와 모바일/콘솔 계정을 여러 개 반복 등록할 수 있습니다.
친구의 요청이나 승인 대기 절차는 없습니다.

등록하면 Java 또는 Floodgate 화이트리스트에도 자동으로 추가됩니다. 친구에게 op를
